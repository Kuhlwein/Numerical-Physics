...
function<double(vec,vec,vec)> linesearch_fancy(function<vec(vec)> f) {
	return [f](const vec x, vec Dx, vec fx) {
		double s=1;
		while(norm(f(x+Dx*s))>(1-s/2)*norm(fx) && s>0.02) {
			double g0=norm(fx)*norm(fx)/2;
			double gprime0=-norm(fx)*norm(fx);

			vec fxs=f(x+Dx*s);
			double gtrial=norm(fxs)*norm(fxs)/2;
			double c = (gtrial-g0-gprime0*s)/(s*s);
